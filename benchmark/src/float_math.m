...
int main(int argc, char **argv)
    ...
    @autoreleasepool {
        ...
        for (uint32_t r = 0; r < 400000; r++)
            for (uint32_t i = 0; i < N; i++) acc = acc + (double)(a[i] * b[i]);
        int64_t t1 = bench_now_us();
        // (uint32_t)acc is UNDEFINED once the sum exceeds u32; via uint64_t
        // the narrowing is defined and wraps. See float_math.xc.
        printf("%u %lld\n", (uint32_t)(uint64_t)acc, (long long)(t1 - t0));
    }
    return 0;
    }
