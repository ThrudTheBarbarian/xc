// test_hiddeninherit_win32.xc — the Win32 half of hiding a container hides its NATIVE children too.
//
...
void main(void)
    ...
    if (!d.boot(&sw, &sh))
        {
        Stdio.printf("SKIP: no Win32 boot\n");
        return;
        }
    ...
    if (gFails == (i32)0)
        {
        Stdio.printf("PASS: hidden is inherited by native descendants (Win32)\n");
        }
    else
        {
