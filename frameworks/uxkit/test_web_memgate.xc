// test_web_memgate.xc — the per-backend memory gate for the web backend (design doc §5 + §10).
//
...
//   1. the driver's NATIVE-OBJECT counter (gDriver.liveNativeCount()) — a leaked canvas-side object, which
//      the heap probe below cannot see (it lives in the JS shim's tables).
...
//
// This is the web instance of the same gate the GEM backend passes (test_memgate.xc).  Each
...
void main(void)
    ...
    if (leaked == (i32)0)
        {
        Stdio.printf("PASS: web backend passes its §10 gate\n");
        }
    else
        {
