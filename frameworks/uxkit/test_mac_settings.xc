// test_mac_settings.xc — UXKit's settings in NSUserDefaults.
//
// The same script as test_gem_settings/test_win32_settings, against the AppKit backend: two runs of
// one binary, the second of which must read back everything the first wrote.  On macOS the store IS
// NSUserDefaults — a suite per domain — so a preference UXKit saves is visible to `defaults read` like
// any other application's, which is the point of using the system's own store rather than a file.
//
// No NSApplication and no window: the settings seam needs neither.
#import <Stdio.xc>
...
void writePass(void)
    ...
    Stdio.printf("  wrote settings to NSUserDefaults\n");
    }
...
void readPass(void)
    ...
    ks.invalidate();
    check("rewrite reaches the defaults", ks.intFor((u8*)"fontSize"), (i32)22);
    ks.setInt((u8*)"fontSize", (i32)14);
    }
...
void main(void)
    ...
    gDriver = drv; // no boot(): settings need no NSApplication

    u8 probe[64];
    checkTrue("the defaults accepted a write", gDriver.settingSet((u8*)"xg.test", (u8*)"probe", (u8*)"ok"));
    ...
    if (gFails == (i32)0)
        {
        Stdio.printf("PASS: settings persist in NSUserDefaults\n");
        }
    else
        {
