...
{ [ -n "$BT" ] && [ -n "$NDKBIN" ] && [ -x "$ADB" ]; } || { echo "== android-loop: skipped (no SDK/NDK) =="; exit 0; }
"$ADB" get-state >/dev/null 2>&1 || { echo "== android-loop: skipped (no device) =="; exit 0; }

work=$(mktemp -d); trap 'rm -rf "$work"' EXIT
echo "== android-loop: building the app lib (pure xcc) + the shim (NDK) =="
"$xcc" -A android --emit-apk -I "$here" "$here/test_android_loop.xc" -o "$work/xtapp.apk" -q 2>/dev/null
...
echo "== android-loop: packaging the two-lib APK =="
cat > "$work/AndroidManifest.xml" <<'EOF'
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.compile_xc.uxloop">
  <uses-sdk android:minSdkVersion="26" android:targetSdkVersion="35"/>
  <application android:hasCode="true" android:label="uxloop">
...
"$BT/apksigner" sign --ks "$KS" --ks-pass pass:uxkit1 --out "$work/uxloop.apk" "$work/aligned.apk" 2>/dev/null
...
"$ADB" logcat -c
"$ADB" shell am start -n org.compile_xc.uxloop/android.app.NativeActivity >/dev/null 2>&1
sleep 8
