...
{ [ -n "$BT" ] && [ -n "$NDKBIN" ] && [ -x "$ADB" ]; } || { echo "== android-real: skipped (no SDK/NDK) =="; exit 0; }
"$ADB" get-state >/dev/null 2>&1 || { echo "== android-real: skipped (no device) =="; exit 0; }

work=$(mktemp -d); trap 'rm -rf "$work"' EXIT
echo "== android-real: building the app lib (pure xcc) + the shim (NDK) =="
"$xcc" -A android --emit-apk -I "$here" "$here/test_android_real.xc" -o "$work/xtapp.apk" -q 2>/dev/null
...
echo "== android-real: packaging the two-lib APK =="
cat > "$work/AndroidManifest.xml" <<'EOF'
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.compile_xc.uxreal">
  <uses-sdk android:minSdkVersion="26" android:targetSdkVersion="35"/>
  <application android:hasCode="true" android:label="uxreal">
...
"$BT/apksigner" sign --ks "$KS" --ks-pass pass:uxkit1 --out "$work/uxreal.apk" "$work/aligned.apk" 2>/dev/null
...
"$ADB" logcat -c
"$ADB" shell am start -n org.compile_xc.uxreal/android.app.NativeActivity >/dev/null 2>&1
sleep 6
