#!/bin/bash
# m68ko3-diff.sh — the ported m68k back end on OPTIMISED IR (task #48).
#
# m68k-diff runs at -O0, which is not the level anybody ships: `make corpus`
...
exec bash "$(dirname "$0")/m68k-diff.sh" 3 "$@"
