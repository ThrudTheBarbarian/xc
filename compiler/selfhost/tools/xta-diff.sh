#!/bin/bash
# xta-diff.sh — the ported 6502 assembler against xta.
# =================================================================
#
# self-hosting M24. Both assemblers get the same `.asm` — the back end's real
# output, includes and all — and their XEX files are compared byte for byte.
# That covers preprocessing, both passes, the long-branch rewriting, the
# banking and the INITAD preload protocol at once.
#
#   bash selfhost/tools/xta-diff.sh [pattern]
...
PATTERN=${1:-}
WORK=${TMPDIR:-/tmp}/xtadiff.$$
mkdir -p "$WORK"
trap 'rm -rf "$WORK"' EXIT

echo "building xta6502 (xtc → native arm64 host binary)…"
"$BIN/xcc" -O2 -A arm64 -H . -o "$WORK/xta6502" selfhost/tools/xta6502.xc \
    -I selfhost/asm > "$WORK/build.log" 2>&1
if [ ! -x "$WORK/xta6502" ]; then grep -a error "$WORK/build.log" | head -5; exit 1; fi

LAYOUT=support/xt6502/layouts/xt.lnk
BANKARGS=(-b -code-window '$6000-$9FFF' -data-window '$A000-$CFFF'
          -code-reg '$D5C0' -data-reg '$D5C1' -I . -I support)
...
for f in $FILES; do
    ...
    if ! "$BIN/xcc" -m xt -H . "${RUN_INCS[@]}" -S -o "$WORK/a.asm" "$f" >/dev/null 2>&1 \
       || [ ! -s "$WORK/a.asm" ]; then
...
echo "--- xta-diff: pass=$pass fail=$fail oracle-failed=$oracle ---"
