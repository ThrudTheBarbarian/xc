#!/bin/bash
# wasmo3-diff.sh — the ported wasm32 back end on OPTIMISED IR (task #48).
#
# wasm-diff runs at -O0 and -O2, which is not the level anybody ships: `make corpus`
...
exec bash "$(dirname "$0")/wasm-diff.sh" O3 "$@"
