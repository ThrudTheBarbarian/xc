#!/bin/bash
# m68k-diff.sh — the ported 68000/68030 back end against xcc-cg-68k.
# =================================================================
#
# self-hosting M15. The third back end after the A9's (M9) and the host's
# (M14), and the first to reuse the shared homing allocator rather than
# arm64's bespoke one.
#
# The oracle is `xcc-cg-68k -O0` over the same IR: the assembly text must come
...
# the A9.
#
#   bash selfhost/tools/m68k-diff.sh [pattern]
...
echo "building xtcg68 (xtc → native arm64 host binary)…"
"$BIN/xcc" -O2 -A arm64 -H . -o "$WORK/xtcg68" selfhost/tools/xtcg68.xc \
...
for f in $FILES; do
    ...
    if ! "$BIN/xcc-fe" -m atarist -H . "${RUN_INCS[@]}" "$f" -o "$WORK/a.ir" \
         >/dev/null 2>&1 || [ ! -s "$WORK/a.ir" ]; then
        oracle=$((oracle+1)); continue
    fi
    if ! "$BIN/xcc-cg-68k" "-O$LEVEL" -q -o "$WORK/a.s" "$WORK/a.ir" >/dev/null 2>&1; then
        oracle=$((oracle+1)); continue
    fi
    IN="$WORK/a.ir"
    if [ "$LEVEL" != 0 ]; then
        "$BIN/xcc-cg-68k" "-O$LEVEL" --dump-opt-ir -q "$WORK/a.ir" > "$WORK/opt.ir" 2>/dev/null \
            || { oracle=$((oracle+1)); continue; }
        IN="$WORK/opt.ir"
    fi
    "$WORK/xtcg68" "$IN" -o "$WORK/b.s" >/dev/null 2>&1
    rc=$?
...
echo "--- m68k-diff: pass=$pass fail=$fail unsupported=$unsup oracle-failed=$oracle ---"
