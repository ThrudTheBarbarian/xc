#!/bin/bash
# x86o3-diff.sh — the ported x86-64 back end on OPTIMISED IR (task #48).
#
# x86-diff runs at -O0, which is not the level anybody ships: `make corpus`
...
exec bash "$(dirname "$0")/x86-diff.sh" 3 "$@"
