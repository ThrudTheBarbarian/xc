// xtast.xc — parse a file with the xtc parser and dump the AST.
// =================================================================
//
// self-hosting M5. The counterpart of `xtc-fe --dump-ast`;
// selfhost/tools/ast-diff.sh runs both and diffs the trees.
//
//   xtast [-I dir]… [-D name=value]… [--raw] <file.xc>
//
...
void main(void)
    ...
    if (input == 0)
        {
        Stdio.printf("usage: xtast [-I dir] [-D name=value] [--raw] <file.xc>\n");
        Process.exit((i32)2);
        return;
        }
    ...
    if (source == 0)
        {
        Stdio.printf("xtast: cannot read '%s'\n", input.cString());
        Process.exit((i32)1);
        return;
        }
