...
// …and one to STDERR. Defined here rather than in the runtime because the
// runtime `.s` files are GENERATED from rt.c by clang and checked in: a
// hand-added stub in one would be lost the next time it was regenerated.
// `write` comes from libSystem, which every arm64 image already loads, so
// this costs an import and nothing else.
//
// A diagnostic must not share the stream a DUMP is written on:
// `xcc-fe --dump-ast` writes to stdout, and a warning landing in the middle
// of it corrupts an artefact the differentials compare
// (private:docs/Design/static-analysis.md §1).
i32 write(i32 fd, u8* buf, i32 len);
void _putc_err(u8 c)
{
    u8 b = c;
    write((i32)2, &b, (i32)1);
...
class Stdio
{
    ...
    {
        if (v >= (u32)10) { Stdio._emitU32(v / (u32)10); }
    ...
    {
        if (v >= (u64)10) { Stdio._emitU64(v / (u64)10); }
    ...
    static void _emitHex(u32 v, u8 digits)
    {
        ...
        while (i != (u8)0)
        {
            ...
            if (nib < (u8)10) { _putc(nib + (u8)$30); }
            else              { _putc(nib + (u8)$37); }   // 'A'-10
        }
    }
    ...
    static void print(string s) { Stdio._emitStr(s); }
#if HAS_ATFMT
    // String* overload — forwards through the wrapped c-string. Gated
    // on HAS_ATFMT so non-`%@` programs don't pull String into scope.
    static void print(String* s) { Stdio._emitStr(s.cString()); }
#endif
    static void print(u16 val)  { Stdio._emitU32((u32)val); }
    static void print(i16 val)  { Stdio._emitI32((i32)val); }
    static void print(u32 val)  { Stdio._emitU32(val); }
    static void print(i32 val)  { Stdio._emitI32(val); }
    static void print(float f)  { _xtc_pf(f); }   // 6dp, matches fp2Asc
    static void print(double d) { _xtc_pd(d); }   // 10dp, matches dp2Asc
    static void print(float f,  u8 precision) { _xtc_pfp(f, precision); }
    static void print(double d, u8 precision) { _xtc_pdp(d, precision); }

    static void printHex(u8 n)   { Stdio._emitHex((u32)n, (u8)2); }
    static void printHex(u16 val){ Stdio._emitHex((u32)val, (u8)4); }
    static void printHex(u32 val){ Stdio._emitHex(val, (u8)8); }
    static void printHex(u64 val){ Stdio._emitHex64(val, (u8)16); }

    // Cursor positioning has no meaning on a byte stream.
    static void setCursor(u8 x, u8 y) { }
    static void init(void) { }
    static void printStruct(void) { _putc((u8)$3F); }      // '?' (TODO)

    // ── printf ───────────────────────────────────────────────────
    // ── field widths ────────────────────────────────────────────────
    // printf parsed `%`, an optional `.` precision, then the conversion —
    // there was no handling of FLAGS or WIDTH at all. So `%10s` consumed `%1`,
    // fell through as unknown, emitted "0s" as ordinary text, and consumed NO
    // argument: every following argument then shifted by one, and `%d` printed
    // the string pointer's low half. Three symptoms — literal specifier tail,
    // no padding, corrupted following arguments — from one cause.
    //
    // Padding needs the rendered LENGTH before the value is written, and each
    // conversion here writes straight out, so these measure first.
    static u16 _lenStr(string s)
    {
        u16 n = (u16)0;
        while (*s != 0) { n = n + (u16)1; s = s + 1; }
        return n;
    }
    ...
    {
        u16 n = (u16)1;
        while (v >= (u64)10) { v = v / (u64)10; n = n + (u16)1; }
        return n;
    }

    static u16 _lenI64(i64 v)
    {
        if (v < (i64)0) return _lenU64((u64)(0 - v)) + (u16)1;
        return _lenU64((u64)v);
    }

    static void _padN(u16 n, u8 c)
    {
        while (n > (u16)0) { _putc(c); n = n - (u16)1; }
    }
    ...
    static void printf(string fmt, ...)
    {
        ...
        u8 padc;        // ' ' normally, '0' when the '0' flag is given.
        ...
        while (ch != 0)
        {
            if (ch == $25)                  // '%'
            {
                ...
                // EVERY conversion — including the ones whose padding is not
                // applied below — because consuming the specifier correctly is
                // what stops the argument list shifting.
                left = 0;
                padc = $20;                 // ' '
                ...
                // Each measured conversion pads to `width`: left by default,
                // right under the '-' flag. A width of 0 makes every _padN a
                // no-op, so an unwidened format costs one compare per
                // conversion and emits exactly what it did before.
                if (spec == $25)            { _putc($25); }              // %%
                else if (spec == $64)                                    // %d
                {
                    i16 dv = va_arg_i16(ap);
                    u16 dl = Stdio._lenI64((i64)dv);
                    if (left == 0 && width > dl) Stdio._padN(width - dl, padc);
                    Stdio.print(dv);
                    if (left != 0 && width > dl) Stdio._padN(width - dl, $20);
                }
                else if (spec == $75)                                    // %u
                {
                    u16 uv = va_arg_u16(ap);
                    u16 ul = Stdio._lenU64((u64)uv);
                    if (left == 0 && width > ul) Stdio._padN(width - ul, padc);
                    Stdio.print(uv);
                    if (left != 0 && width > ul) Stdio._padN(width - ul, $20);
                }
                else if (spec == $78)       { Stdio.printHex(va_arg_u16(ap)); } // %x
                else if (spec == $6C)                                    // %l...
                {
                    fmt  = fmt + 1;
                    spec = *fmt;
                    if (spec == $6C)                                     // %ll...
                    {
                        fmt  = fmt + 1;
                        spec = *fmt;
                        if (spec == $64)      { Stdio._emitI64(va_arg_i64(ap)); } // %lld
                        else if (spec == $75) { Stdio._emitU64(va_arg_u64(ap)); } // %llu
                        else if (spec == $78) { Stdio.printHex(va_arg_u64(ap)); } // %llx
                    }
                    else if (spec == $64)                                // %ld
                    {
                        i32 lv = va_arg_i32(ap);
                        u16 ll = Stdio._lenI64((i64)lv);
                        if (left == 0 && width > ll) Stdio._padN(width - ll, padc);
                        Stdio.print(lv);
                        if (left != 0 && width > ll) Stdio._padN(width - ll, $20);
                    }
                    else if (spec == $75)                                // %lu
                    {
                        u32 luv = va_arg_u32(ap);
                        u16 lul = Stdio._lenU64((u64)luv);
                        if (left == 0 && width > lul) Stdio._padN(width - lul, padc);
                        Stdio.print(luv);
                        if (left != 0 && width > lul) Stdio._padN(width - lul, $20);
                    }
                    else if (spec == $78)   { Stdio.printHex(va_arg_u32(ap)); } // %lx
                    else if (spec == $66)   { Stdio.print(va_arg_double(ap), prec); } // %lf
                }
                else if (spec == $63)                                    // %c
                {
                    u8 cv = va_arg_u8(ap);
                    if (left == 0 && width > (u16)1) Stdio._padN(width - (u16)1, padc);
                    _putc(cv);
                    if (left != 0 && width > (u16)1) Stdio._padN(width - (u16)1, $20);
                }
                else if (spec == $73)                                    // %s
                {
                    string sv = va_arg_string(ap);
                    u16 sl = Stdio._lenStr(sv);
                    if (left == 0 && width > sl) Stdio._padN(width - sl, padc);
                    Stdio.print(sv);
                    if (left != 0 && width > sl) Stdio._padN(width - sl, $20);
                }
                else if (spec == $66)       { Stdio.print(va_arg_float(ap), prec); }  // %f
                else if (spec == $65)       { Stdio.print(va_arg_u16(ap)); }    // %e (number)
#if HAS_ATFMT
                else if (spec == $40)                                    // %@
                {
    ...
    static void printfAt(u8 x, u8 y, string fmt, ...)
    {
        ...
        while (ch != 0)
        {
            if (ch == $25)
            {
                ...
                spec = *fmt;
                if (spec == $25)            { _putc($25); }
                else if (spec == $64)       { Stdio.print(va_arg_i16(ap)); }
                else if (spec == $75)       { Stdio.print(va_arg_u16(ap)); }
                else if (spec == $78)       { Stdio.printHex(va_arg_u16(ap)); }
                else if (spec == $6C)
                {
                    fmt  = fmt + 1;
                    spec = *fmt;
                    if (spec == $64)        { Stdio.print(va_arg_i32(ap)); }
                    else if (spec == $75)   { Stdio.print(va_arg_u32(ap)); }
                    else if (spec == $78)   { Stdio.printHex(va_arg_u32(ap)); }
                    else if (spec == $66)   { Stdio.print(va_arg_double(ap)); }
                }
                else if (spec == $63)       { _putc(va_arg_u8(ap)); }
                else if (spec == $73)       { Stdio.print(va_arg_string(ap)); }
                else if (spec == $66)       { Stdio.print(va_arg_float(ap)); }
                else if (spec == $65)       { Stdio.print(va_arg_u16(ap)); }
            }
            else
            {
                _putc(ch);
