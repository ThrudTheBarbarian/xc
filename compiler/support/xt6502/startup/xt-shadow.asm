; xt-shadow.asm — banked xt startup + shadow ROM
;
; Combines xt's two independent bank selectors ($82 for code over
; $4000-$5FFF, $83 for data over $6000-$7FFF) with the shadow-
; mode ROM-disable machinery so main code can span the usual
; $A000-$CFFF + $D800-$FFF9 (~22 KB) area.
;
; Both bank registers zero at boot — each selector is an 8-bit
; bank id, no legacy 16-bit-pair semantics. Banked accesses write
; the selectors explicitly on demand.
;
...
_shadow_src:
    ...
    ; Initial bank state: no bank paged in
    LDA #$00
    STA {{zp.bankReg}}
    STA {{zp.bankReg}} + 1
