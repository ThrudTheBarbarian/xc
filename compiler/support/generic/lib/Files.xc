...
// =================================================================
//
// Any host with the `_xt_file_*` primitives — arm64 and x86-64 both have
// them. `FILE.xc` in the platform directories
...
// M4).
//
// It is layered on the host C runtime (support/<arch>/runtime/, e.g. arm64's
// libxt.c or x86-64's sys-linux.s), the same
...
//
// Not portable to the other targets yet: xt6502 has no filesystem, and x86_64 /
// win64 have no libxt.c of their own. When they get one this class moves to a
// shared layer rather than being copied.
...
i32 _xt_file_chmod_exec(u8* path);
...
class Files
    ...
    // Mark the file executable (0755). A linker that cannot do this leaves an
    // output nobody can run.
    static bool setExecutable(String* path)
        {
        return _xt_file_chmod_exec(path.cString()) != (i32)0;
        }
    ...
    // needs this is a compiler writing a cache under $HOME on a machine that has
    // never run it — so "already exists" is success, not a clash.
