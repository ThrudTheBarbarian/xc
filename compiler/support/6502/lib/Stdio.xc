...
class Stdio
    ...
    static void print(float f, u8 precision) XTC_CLOAKED
        ...
        while (ch != 0)
            {
            if (precision != 0 && sawDot != 0)
                {
                if (dpCount >= precision)
                    {
                    return;
                    }
                dpCount = dpCount + 1;
    ...
    //
    // Gated on HAS_LFMT so programs that never print doubles don't
    // pay the ~800-byte dp2Asc cost in main-region space.
...
#endif
// The 32-bit `%ld`/`%lu`/`%lx` branches share a radix macro with their
// 16-bit `%d`/`%u`/`%x` form (HAS_DFMT/HAS_UFMT/HAS_XFMT): the compiler
// owns the width and rewrites `%d`→`%ld` for a 32-bit argument, so both
// widths of a radix are compiled in together. HAS_LFMT (%lf/double) is a
// separate KIND and stays independent.
#if HAS_DFMT || HAS_UFMT || HAS_XFMT || HAS_LFMT
                else if (spec == $6C)
                    {
                    // %l — 32-bit prefix
                    fmt = fmt + 1;
                    ...
                    if (spec == $00)
                        ...
                        {
                        // %lf — double. Only compiled in when the
                        // driver's pre-scan detects %lf in the input
                        // (or the user passed -DHAS_LFMT=1). Pass
