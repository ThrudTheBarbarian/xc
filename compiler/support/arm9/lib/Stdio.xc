...
// Float formatting goes straight to libc snprintf (auto-imported on arm9 — no
// #import <c> needed), so there's no xtc-vended _xtc_p* C formatter to ship. See
// emitFloat below. snprintf is variadic but C-ABI (AAPCS), so it never touches
// the xtc varargs pack buffer — calling it from Stdio.printf is safe.
...
class Stdio
    ...
    // Float / double → fixed-point ASCII. libc snprintf ROUNDS, but the cross-arch
    ...
    static void emitFloat(double v, u8 prec)
        {
        u8 buf[96];
        i32 n = snprintf(&buf[0], (u32)96, "%.*f", (i32)prec + (i32)8, v);
        if (n <= (i32)0)
            {
            return;
            }
