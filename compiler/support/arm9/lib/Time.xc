...
// Time.xc — arm64 host port.
//
// The Atari port drives the RTCLOK jiffy counter; the host has none, so the
// timer reads a monotonic wall clock. The implementation lives in libxt.a
// (support/arm64/runtime/libxt.c) under standardized names; this class just
// calls those primitives and does the arithmetic — no stubbed no-ops. The
// compiler links libxt.a automatically on platforms that need a host runtime.
//
