#!/bin/sh
# run_rocks_model.sh — the `rocks-rsc` gate: the resource model and the
...
xcc=${XCC:-xcc}
ARCH="${1:-arm64}"
command -v "$xcc" >/dev/null 2>&1 || { echo "== rocks-rsc: no compiler ('$xcc'); set XCC =="; exit 2; }
...
case "$ARCH" in win64) OUT="$work/rkrsc.exe" ;; *) OUT="$work/rkrsc" ;; esac
LIB=""
...
case "$ARCH" in
  arm64) SRC="$here/xc/test_rkrsc.xc" ;;
  *)     SRC="$here/xc/compile_check_rsc.xc" ;;
esac
"$xcc" -A "$ARCH" -I "$ux" -I "$here/xc" $LIB "$SRC" -o "$OUT" -q
case "$ARCH" in
  arm64) ;;
  *) echo "== rocks-rsc ($ARCH): COMPILE OK (no Files.xc on this target) =="; exit 0 ;;
esac
...
if [ "$ARCH" = arm9 ]; then
  echo "== rocks-rsc (arm9): BUILD OK (no host runner) =="; exit 0
fi
case "$ARCH" in
